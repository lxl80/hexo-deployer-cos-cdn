...
const TeoPurgeUrlsCache = require('./tencent_cloud');
...
function deploy(localFileMap, cfgs, isImageFolder, publicDir) {
    ...
    const cos = new COS({
        ...
        UseAccelerate: cfgs.useAccelerate
    });
                    ...
                    return cacheRefresh(cfgs, filesMap)
                        .then((res) => {
                            if (res !== false) {
                                ...
                                console.log(chalk.red('刷新CDN缓存失败！'));
                            }
                        })
                        .catch((err) => {
                            console.log(chalk.red('刷新CDN缓存失败！'));
...
function cacheRefresh(cfgs, filesMap) {
    return new Promise((resolve, reject) => {
        ...
        if (cfgs.cloud === 'tencent') {
            ...
            if (cfgs.cdnType === 'teo') {
                return TeoPurgeUrlsCache(cfgs.secretId, cfgs.secretKey, Object.values(urls));
            }else{
                QCSDK.config({
                    secretId: cfgs.secretId,
                    secretKey: cfgs.secretKey
