...
如上配置，还安装了 git 部署插件 [hexo-deployer-git](https://www.npmjs.com/package/hexo-deployer-git)，会同步将网站发布到 Github Pages 及 COS 中；如果 bucket 中有多余文件会删除；图片上传成功后，会将 Markdown 源文件中图片路径替换为 CDN 地址。
